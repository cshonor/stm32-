...
third_party/  git 子模块：libopencm3（labs/02b 用；clone 后先 git submodule update --init）
...
| 旁支 | 库路线：libopencm3（≠ HAL） | labs/02b-libopencm3-blink | ✅ 主机侧实测 |
...
- 调试：ST-Link V2
  ...
  - 烧录配置两份：`labs/02b-libopencm3-blink/openocd/f103rb.cfg`（NUCLEO 板载 ST-Link）、
...
- 交叉链（两条，按 lab 选）：
  ...
  - **GNU 一条**（labs/02b 及以后用库的场合）：Arm GNU Toolchain 14.2.Rel1
